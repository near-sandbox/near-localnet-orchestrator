...
class ConfigManager {
    constructor(configPath, logger) {
        this.logger = logger || new Logger_1.Logger('ConfigManager');
    ...
    async load() {
        try {
            ...
            const config = (0, schema_1.validateConfig)(rawConfig);
     ...
     */
    validate(config) {
        const result = (0, schema_1.validateConfigSafe)(config);
        if (result.success) {
     ...
     */
    getLayerDependencies(layerName) {
        if (!this.configCache) {
            throw new Error('Configuration not loaded. Call load() first.');
        }
     ...
     */
    topologicalSort(layerNames, allLayers) {

...
class NearServicesLayer extends BaseLayer_1.BaseLayer {
     ...
     */
    async deployContractsViaSSM(nearOutputs) {
        ...
        try {
            ...
            throw new Error(`Failed to parse SSM command ID from JSON output: ${error?.message || String(error)}`);
        }
    ...
    async getOutputs() {
        ...
        try {
            ...
            let faucetOutputs = {};
            try {
                faucetOutputs = await this.readStackOutputs('near-localnet-faucet-v6');
            }
            catch (error) {
                // Back-compat: older deployments may still be on v5/v4/v3/v2
                try {
                    faucetOutputs = await this.readStackOutputs('near-localnet-faucet-v5');
                }
                catch (error) {
                    try {

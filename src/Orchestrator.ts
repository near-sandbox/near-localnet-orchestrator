...
import { NearBaseLayer } from './layers/NearBaseLayer';
...
export class Orchestrator {
  ...
  private stackReader: StackOutputReader;
  private commandExecutor: CommandExecutor;

  private config?: SimulatorsConfig;
  private layerOutputs: Map<string, LayerOutput> = new Map();
  ...
  constructor(options: OrchestratorOptions = {}) {
    ...
    this.gitManager = new GitManager(this.getWorkspaceRoot(), this.logger);
    ...
    this.cdkManager = new CdkManager(this.logger);
    this.stackReader = new StackOutputReader(this.logger);

    // Initialize config manager
    this.configManager = new ConfigManager(configPath, this.logger);
   ...
   */
  async run(targetLayers?: string[]): Promise<{ success: boolean; error?: string }> {
    if (!this.config) {
   ...
   */
  async verify(targetLayers?: string[]): Promise<{ success: boolean; results: Record<string, VerifyResult> }> {
    if (!this.config) {
      throw new Error('Orchestrator not initialized. Call initialize() first.');
    }
   ...
   */
  async destroy(targetLayers?: string[]): Promise<{ success: boolean; error?: string }> {
    if (!this.config) {
      throw new Error('Orchestrator not initialized. Call initialize() first.');
    }
   ...
   */
  getStatus(): { layers: Record<string, LayerOutput>; timestamp: string } {
    return {
   ...
   */
  private async processLayer(layerName: string): Promise<{ success: boolean; deployed: boolean }> {
    const layer = this.createLayerInstance(layerName);
   ...
   */
  private async destroyLayer(layerName: string): Promise<boolean> {
    try {
      const layer = this.createLayerInstance(layerName);
  ...
  private createLayerInstance(layerName: string): BaseLayer {
    ...
    switch (layerName) {
      ...
      case 'near_services':
        return new NearServicesLayer(layerName, context);
      case 'chain_signatures':
        return new ChainSignaturesLayer(layerName, context);
   ...
   */
  private resolveDependencies(targetLayers: string[]): string[] {
    if (!this.config) {
      throw new Error('Configuration not loaded');
    }
    ...
    const allRequiredLayers = new Set<string>();
    const collectDependencies = (layerName: string) => {
      if (allRequiredLayers.has(layerName)) return;
    ...
    return this.configManager.getEnabledLayers().filter(layer =>
      allRequiredLayers.has(layer)
    );
  }
   ...
   */
  private async rollbackFailedLayers(deployedThisRun: string[]): Promise<void> {
    const layersToRollback = [...deployedThisRun].reverse();
    if (layersToRollback.length === 0) return;
   ...
   */
  private async rollbackAllLayers(): Promise<void> {
  ...
  private async loadDeploymentState(): Promise<void> {
    try {
      ...
      if (fs.existsSync(statePath)) {
        ...
        const savedState = JSON.parse(stateContent) as DeploymentState;
   ...
   */
  private async cleanupDeploymentState(): Promise<void> {
    try {
      const statePath = path.join(process.cwd(), 'deployment-state.json');
